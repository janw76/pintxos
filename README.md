...
Versions look like `26.09.1`: year, month, and a number that starts at 1 and
increments with each release in that month, resetting to 1 next month. For
example `26.09.1`, `26.09.2`, then `26.10.1`. Each release is tagged in git
as `vYY.MM.N`; the app displays the version without the leading `v`.
Pintxøs doesn't use semantic versioning — it's an app, not a library, so
API-compatibility semantics wouldn't mean anything here.

Every merge to `main` is a release. CI runs the tests, works out the next
number for the current month from the existing tags, builds and pushes the
Docker image as `ghcr.io/janw76/pintxos:<version>` and `:latest`, tags the
commit `v<version>`, and creates a GitHub Release. The release notes are
generated by GitHub from the merged pull requests' titles and bodies since
the previous tag, plus a collapsed list of commits.

The running app shows its version in the footer of the Settings page. The
image build bakes it in as the `PINTXOS_VERSION` environment variable; when
